...
def homescreen_function():
    ...
    def open_foodwindow():
        if not hasattr(homepage, 'food_window') or homepage.food_window is None or not homepage.food_window.winfo_exists():
            homepage.food_window = FoodPage(homepage)  # create window if it doesn't exist or is destroyed
        else:
            homepage.food_window.focus()  # if window exists, focus it

    food_button = ctk.CTkButton(master=entry_frame, text="Open Food Window", command=open_foodwindow,
                                corner_radius=6, fg_color="#FFC300", font=('Switzer', 16, 'bold'))
